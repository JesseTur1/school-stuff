...
void tallennaPeli(string tiedostoNimi, MOBIILIPELI peliNimi)
{
	ofstream tallennus(tiedostoNimi, std::ifstream::app);
	if (tallennus)
	{
		tallennus << peliNimi.nimi << endl;
		tallennus << peliNimi.julkaisija << endl;
		tallennus << peliNimi.hinta << endl;
		tallennus.close();
	}
}

void tulostapeli(MOBIILIPELI pelinimi)
{
	cout << pelinimi.hinta << " <- hinta" << endl;
	cout << pelinimi.nimi << " <- nimi" << endl;
	cout << pelinimi.julkaisija << " <- julkaisija" << endl;
}

void tarkistaPeli(string tiedostoNimi)
{
	ifstream luku(tiedostoNimi);
	if (luku)
	{
		while (luku.peek() != EOF)
		{
			MOBIILIPELI valipeli = { "","",0 };
			string tmp;

			getline(luku, valipeli.nimi, '\n');
			getline(luku, valipeli.julkaisija, '\n');
			getline(luku, tmp, '\n');
			valipeli.hinta = atof(tmp.c_str());

			tulostapeli(valipeli);
...

{
	...
	{
		MOBIILIPELI peli1 = { };
		cout << "kerro pelisi nimi" << endl;
		cin >> peli1.nimi;
		cout << "kerro pelisi julkaisija" << endl;
		cin >> peli1.julkaisija;
		cout << "kerro pelisi hinta. ei desimaaleja just nyt ty" << endl;
		cin >> peli1.hinta;
		tallennaPeli("pelit.txt", peli1);
		cout << "onnistuneesti done. luultavasti. En tarkistanut." << endl;
		cout << "tarkista ite." << endl;

		tarkistaPeli("pelit.txt");
